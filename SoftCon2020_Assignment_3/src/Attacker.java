...
public class Attacker {
    public void AIAttack(Grid playergrid, ArrayList<Ship> playerList, Scoreboard scoreboard, Player player, Enemy enemy){
        ...
        if (playergrid.grid[ShotV][ShotH] != "[X]" && playergrid.grid[ShotV][ShotH] != "[O]" ){
            System.out.println("The computer attacks position " + Transformer.IntToStringTransform1(ShotV, ShotH));
            if(playergrid.grid[ShotV][ShotH] == "[ ]"){
                ...
                for (int i = 0; i < 10; i++) {
                    ...
                    for (int j=size_coordlist-1; j>=0; j--){
                        if (coordlist.get(j).equals(Integer.toString(ShotH) + Integer.toString(ShotV))) {
                            coordlist.remove(j);
